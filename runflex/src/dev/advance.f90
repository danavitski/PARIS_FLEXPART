...
subroutine advance(itime, nrelpoint, ldt, up, vp, wp, usigold, vsigold, wsigold, nstop, xt, yt, zt, prob, icbt, ipart)
!                     i    i  i/oi/oi/o
...
!7      th(i)=real(i)/real(iclass)
    ...
    if (nglobal.and.(yt>switchnorthg)) then
        ngrid=-1
    else if (sglobal.and.(yt<switchsouthg)) then
        ngrid=-2
    else
        ngrid=0
        do j=numbnests,1,-1
            if ((xt>xln(j)+eps).and.(xt<xrn(j)-eps).and. (yt>yln(j)+eps).and.(yt<yrn(j)-eps)) then
                ngrid=j
                exit
            endif
        end do
    ...
    if (abs(itime-memtime(1))<abs(itime-memtime(2))) then
        memindnext=1
    else
        memindnext=2
    endif
    ...
    if (ngrid<=0) then
        do k=1,2
            ...
            if (interpolhmix) then
                ...
                do j=jy,jyp
                    do i=ix,ixp
                        if (hmix(i,j,1,mind)>h) h=hmix(i,j,1,mind)
                    end do
                end do
            endif
        end do
        ...
        do k=1,2
            ...
            do j=jy,jyp
                do i=ix,ixp
                    if (hmixn(i,j,1,mind,ngrid)>h) h=hmixn(i,j,1,mind,ngrid)
                end do
            end do
        end do
...
!        loop=0
    !100     loop=loop+1
            ...
            if (nrand+ifine>maxrand) nrand=1
            rhoaux=rhograd/rhoa
            dtf=dt*fine
            dtftlw=dtf/tlw
            ...
            !*************************************************************
                ...
                if (turbswitch) then
                    if (dtftlw<.5) then
                        ...
                        if (cblflag==1) then  !modified by mc
                            if (-h/ol>5) then  !modified by mc
                ...
                if (abs(delz)>h) delz=mod(delz,h)

                ! Determine if particle transfers to a "forbidden state" below the ground
                ! or above the mixing height
                !************************************************************************

                if (delz<-zt) then         ! reflection at ground
                    icbt=-1
                    zt=-zt-delz
                else if (delz>(h-zt)) then ! reflection at h
                    icbt=-1
                    zt=-zt-delz+2.*h
                else                         ! no reflection
                    icbt=1
            ...
            if (mdomainfill==0) then
                if (lsettling) then
                    do nsp=1,nspec
                        if (xmass(nrelpoint,nsp)>eps2) exit
                    end do
                    if (nsp>nspec) then
    ...
    !          alias interpol_wind (division by zero)
            if (zt>=height(nz)) zt=height(nz)-100.*eps
            if (zt>h) then
                !if (itimec==itime+lsynctime) goto 99
                exit !goto 700    ! complete the current interval above PBL
            endif
    ...
    !   this formulation means that prob(n) = 1 - f(0)*...*f(n)
        !   where f(n) is the exponential term
            ...
            if (zt<0.) zt=min(h-eps2,-1.*zt)    ! if particle below ground -> reflection

            if (itimec==(itime+lsynctime)) then
                usig=0.5*(usigprof(indzp)+usigprof(indz))
                vsig=0.5*(vsigprof(indzp)+vsigprof(indz))
                wsig=0.5*(wsigprof(indzp)+wsigprof(indz))
                exit !goto 99  ! finished
            endif
        enddo !goto 100
...
!700 continue
    ...
    ! Turbulent velocity scales are determined based on sqrt(d_trop/dt)
        ...
        if (zt<tropop) then  ! in the troposphere
            ...
            nrand=nrand+2
            wp=0.
        else if (zt<tropop+1000.) then     ! just above the tropopause: make transition
            weight=(zt-tropop)/1000.
            uxscale=sqrt(2.*d_trop/dt*(1.-weight))
            if (nrand+2>maxrand) nrand=1
            ux=rannumb(nrand)*uxscale
            vy=rannumb(nrand+1)*uxscale
...
!99  continue
    ...
    if (zt<0.) zt=-1.*zt    ! if particle below ground -> refletion
    ...
    if (ngrid>=0) then
    ...
    else if (ngrid==-1) then      ! around north pole
        ...
        yt=(ylat-ylat0)/dy
    else if (ngrid==-2) then    ! around south pole
        xlon=xlon0+xt*dx
        ylat=ylat0+yt*dy
    ...
    if (xglobal) then
        if (xt>=real(nxmin1)) xt=xt-real(nxmin1)
        if (xt<0.) xt=xt+real(nxmin1)
        if (xt<=eps) xt=eps
        if (abs(xt-real(nxmin1))<=eps) xt=real(nxmin1)-eps
    endif

    ! HSO/AL: Prevent particles from disappearing at the pole
    !******************************************************************

    if ((yt < 0. ) .and. (ylat0 == -90)) then
        xt=mod(xt+180.,360.)
        yt=-yt
    else if ((yt > real(nymin1)) .and. (ylat0 + ny * dy == 90)) then
        xt=mod(xt+180.,360.)
        yt=2*real(nymin1)-yt
    endif
    ...
    if ((xt<0.).or.(xt>=real(nxmin1)).or.(yt<0.).or.(yt>real(nymin1))) then
        nstop=3
        return
    endif
    ...
    if (abs(itime+ldt*ldirect)>abs(memtime(2))) return
    ...
    if ((yt < 0.) .and. (ylat0 == -90)) then
        xt=mod(xt+180.,360.)
        yt=-yt
    else if ((yt > nymin1) .and. (ylat0 + ny * dy == 90)) then
        xt=mod(xt+180.,360.)
        yt=2*real(nymin1)-yt
    endif
